...
// MapQuest is used ONLY for map tile rendering and markers — never for geocoding
...
export default function PhysicianTrialMap({ trial, physicians }: Props) {
  ...
  const buildMap = useCallback(async () => {
    ...
    try { await loadMapQuest(); } catch { setStatus("error"); return; }
    if (!containerRef.current) return;
    ...
    // Resolve coordinates for all US trial locations
    // Rule: if data has lat/lon, use it. If not, use Geoapify. Never use MapQuest.
    ...
    // Determine map center from bounding box
    let centerLat = 39.5;
    let centerLon = -98.35;
    let zoom = 4;
    ...
    // Radius circle centered on first trial point
    ...
    // Physician markers — geocode via Geoapify if no coords, filter by radius
    ...
    <div style={{ border: "1.5px solid #dde5f5", borderRadius: "14px", overflow: "hidden", boxShadow: "0 2px 12px rgba(99,102,241,0.06)", fontFamily: "'Inter',sans-serif" }}>
      ...
      {(status === "loading" || status === "geocoding") && (
        <div style={{ height: "420px", background: "#f8faff", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "10px" }}>
          ...
          <div style={{ width: 32, height: 32, border: "3px solid rgba(99,102,241,0.15)", borderTopColor: "#6366f1", borderRadius: "50%", animation: "mq-spin 0.75s linear infinite" }} />
          <span style={{ fontSize: "13px", color: "#94a3b8" }}>
            {status === "geocoding" ? "Locating trial sites via Geoapify..." : "Loading map..."}
          </span>
        </div>
      )}
      ...
      <div ref={containerRef} style={{ height: "420px", width: "100%", display: status === "ready" ? "block" : "none" }} />
    </div>
  );
}